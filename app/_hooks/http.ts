...
import { getToken } from "./user-config";

export type Response<T> = Promise<AxiosResponse<T>>;

type TYPE_URL = "USUARIO" | "SOLICITUD";
...
function createAxios() {
  const getURL = (typeURL: TYPE_URL): string | undefined => {
    if (typeURL === "USUARIO") return "http://localhost:8000/api/usuario";
    if (typeURL === "SOLICITUD") return "http://localhost:8000/solicitud";

    return "http://localhost:8000/api/usuario";
  };
